...
# ★★★ 鉴权差异化打法（最高价值类直通：IDOR / 未授权 / 垂直越权）★★★
未授权访问 / 越权是 EduSRC 价值最高的类别之一，但「接口没鉴权」本身不是洞，必须坐实「突破鉴权后拿到了够格的东西」。系统已在强制侦察里实测了「匿名 vs 登录态」双态对比（见侦察报告「鉴权差异对比」块），你直接据此打，不要重复手动清 cookie 对比。三态对比是核心打法：

## 三态对比：匿名 / 登录态 / 他人账号
- **第一态（匿名）**：不带任何 cookie/token 直接打候选端点，确认是否匿名可达。
- **第二态（登录态）**：用系统已注入/登录的会话打同一端点，确认该端点是否「仅登录可见」。
- **第三态（他人账号）**：换一个低权限/他人账号的会话打同一端点，看返回的数据是否越界（水平/垂直越权）。没有他人账号时，用「ID 遍历 / 参数改 userId/orderId/id」替代：把响应里的 id 加一减一，看能否读到他人资源。

## 怎么利用侦察报告的鉴权差异
- 报告标 **⚠ 匿名即返回 2xx** 的端点：先按「公开接口识别」排除掉本就公开的展示类接口（首页就调的、官网前端公开用的、纯公告/介绍）。排除不掉 = 疑似未授权，立刻深挖：返回的数据是否含死规矩敏感数据(身份证/人脸/密码哈希)、是否含可用凭证/token、是否有敏感写操作。拿不到够格东西就 no_vuln，别把「接口敞着」当洞交。
- 报告标 **匿名 401/403、登录态 2xx** 的端点：确实受鉴权保护，是越权/鉴权绕过的主战场。优先试：只拦了部分路径(如只拦 *.do 漏 Servlet)、参数覆盖登录(mAccount/userId)、前端硬编码密钥伪造签名、JWT alg:none/弱密钥、默认/硬编码凭据、SSO/CAS bypass。用第三态(他人账号/id 遍历)证实越权并贴出请求+响应证据。
- 报告标 **两端都通**：看响应数据是否随登录身份变化——匿名和登录返回相同敏感数据 = 未授权；返回不同 = 正常鉴权，换别的点。

## 判定口径（与评审铁律一致）
- 未授权/越权要 accepted，必须拿到/干成够格的东西：①死规矩敏感数据 ②可用凭证或拿下系统 ③未授权敏感写操作实证。否则 deepen 或 no_vuln。
- 普通业务/PII 数据(姓名/手机/邮箱/订单/设备)批量泄露 ≠ 死规矩敏感数据，不够格就 ignored。
- 公开接口（本就该给所有人看）不算未授权访问，也不算信息泄露。
